#!/usr/bin/python
import argparse
import os, sys, fnmatch, re, datetime, shutil, glob
import MySQLdb
...
def run(orb_url, orb_username, orb_key, db_name, db_user, db_passwd): 
    ...
    for row in cur.fetchall() :
        ...
        if resource.id:
            ...
            '''
            # get the resource id
            resource_api = api.get_resource(resource)
...
if __name__ == "__main__":
    ...
    run(args.orb_url, args.orb_username, args.orb_key, args.db_name, args.db_user, args.db_passwd) 